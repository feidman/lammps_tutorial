...
% eom image.00000.jpg                # view initial snapshot 
% eom image.20000.jpg                # view final snapshot 
% vlc movie.mpg                      # play movie
...
(5) Run on two-processors and compare CPU timing to one-processor run

% mpirun -np 2 lmp -in in.friction

NOTE: The use of "-in" instead of "<" is sometimes needed when running
in parallel, when multiple processors read the input script.

See if the CPU timing (first number) in this line 
is different on 1 proc vs 2 procs:

>> Loop time of 5.65555 on 1 procs for 20000 steps with 1724 atoms

You can use any value for the -np argument up to the number
of cores on your machine.

NOTE: This assumes that when you installed the Virtual Box image on
your machine, you changed the core-count setting to match your
machine.  The default value is 2.
